...
pub fn compare_cpu_infos(v1: Vec<CpuInfo>, v2: Vec<CpuInfo>) -> (Vec<CpuInfo>, HashMap<std::string::String, &'static str>) {
    let mut diff_vec = Vec::new();
    let mut added_removed = HashMap::new();

    let v1_map: HashMap<&str, &CpuInfo> = v1.iter().map(|x| (x.name.as_str(), x)).collect();
    let v2_map: HashMap<&str, &CpuInfo> = v2.iter().map(|x| (x.name.as_str(), x)).collect();
...
pub fn save_stats(file_stats: &str, timestamp: u64, getcpunow: &Vec<CpuInfo>) {
...
fn main() {
    ...
    let getcpunow = get_cpu_infos();
    let mut cpulastdata: Vec<CpuInfo> = Vec::new();
    ...
    if !first_start {
        ...
        // ALERTS 
        if lastchecktime < 2 {
            println!("Interval between two executions is too short, the information may be erroneous. It is advisable to wait at least 2 seconds before a second execution. Interval ({}s)", lastchecktime);
        }
